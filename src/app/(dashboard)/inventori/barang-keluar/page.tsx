...
export default function BarangKeluarPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/inventori/barang-keluar");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/inventori/barang-keluar?id=${id}`, { method: "DELETE" });
      const json = (await res.json()) as any;
  ...
  const exportToExcel = () => {
    const exportData = filteredData.map((item, idx) => ({
      ...
      "Total": item.jumlah * item.hargaJual,
      "Keterangan": item.keterangan || "-"
    }));
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      startY: 20,
      head: [["No", "Nama Barang", "Tanggal", "Kuantiti", "Harga Jual", "Total", "Keterangan"]],
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-text-primary">Barang Keluar</h2>
          <p className="text-text-secondary mt-1">Catat barang yang keluar dari stok.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-surface rounded-lg w-full max-w-4xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center">
              <h3 className="text-lg font-bold text-text-primary">Preview Laporan Barang Keluar</h3>
                ...
                <tbody className="divide-y divide-border">
                  {filteredData.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-6 py-3">{formatRupiah(item.hargaJual)}</td>
                      <td className="px-6 py-3">{formatRupiah(item.jumlah * item.hargaJual)}</td>
                    </tr>
                  ))}
      ...
      {isAdding && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-surface rounded-lg w-full max-w-md shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center">
              <h3 className="text-lg font-bold text-text-primary">{isEditing ? "Edit Barang Keluar" : "Tambah Barang Keluar"}</h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required type="number" className="w-full px-3 py-2 border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
                      value={formData.hargaJual} onChange={e => setFormData({...formData, hargaJual: e.target.value})} />
                  </div>
                  <div>
