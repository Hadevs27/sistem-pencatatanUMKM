...
export default function PenghasilanPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/operasional/penghasilan");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/operasional/penghasilan?id=${id}`, { method: "DELETE" });
      const json = (await res.json()) as any;
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Penghasilan");
    XLSX.writeFile(workbook, "Laporan_Penghasilan.xlsx");
  };

  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text("Laporan Penghasilan", 14, 15);
    autoTable(doc, {
      startY: 20,
    ...
    doc.save("Laporan_Penghasilan.pdf");
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-text-primary">Penghasilan</h2>
          <p className="text-text-secondary mt-1">Catat semua pemasukan di luar penjualan stok utama.</p>
        </div>
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr key={item.idTransaksi || idx} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-success font-medium">{formatRupiah(item.nominal)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-surface rounded-lg w-full max-w-4xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center">
              <h3 className="text-lg font-bold text-text-primary">Preview Laporan Penghasilan</h3>
                ...
                <tbody className="divide-y divide-border">
                  {filteredData.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="px-6 py-3 text-success font-medium">{formatRupiah(item.nominal)}</td>
      ...
      {isAdding && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-surface rounded-lg w-full max-w-md shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center">
              <h3 className="text-lg font-bold text-text-primary">{isEditing ? "Edit Penghasilan" : "Catat Penghasilan"}</h3>
